...
y_labels = unique(y);
y_train = zeros(numel(y), 1);
y_test = zeros(numel(yt), 1);
for i_label = 1 : numel(y_labels)
    y_train(y==y_labels(i_label))=i_label;
    y_test(y==y_labels(i_label))=i_label;
end

X_train = [normrnd(-3,1,[500,1]);normrnd(3,1,[500,1])];
y_train = [ones(500,1);ones(500,1)+1];

% W=normrnd(0,10,[100,2]);
% [X,Y]=meshgrid(W);
